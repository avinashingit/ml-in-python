...
def train_and_test_gnb():
    ...
    train_features, test_features, train_response, test_response = split_data(
        dataFeatures, dataResponse, 0.4)
    ...
    train_accuracy = calculate_accuracy(
        predicted_values=training_predictions, actual_values=train_response)

    # Predict on testing dataset
    testing_predictions = nb_model.predict(test_features)
    test_accuracy = calculate_accuracy(
        predicted_values=testing_predictions, actual_values=test_response)
